...
【3】递归内调用函数获得返回值后，会继续向下执行，直至执行结束。也要遵循if，switch，顺序等的执行结构  
【4】递归和循环完全可以等价，当时递归的代码更加简洁，但是不容易理解  
【5】可以专门设立一个方法，该方法整体就是递归，在主函数中调用该方法即可  
【6】对递归函数进行推导的时候，从退出递归条件开始推，获得每次返回值，并将该返回值从赋值给上一次递归函数，以此类推，直至逆推出调用递归时的初始状态  
